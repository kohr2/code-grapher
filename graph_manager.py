...
class CodeGraphManager:
    ...
    def __init__(self, uri: Optional[str] = None, username: Optional[str] = None, password: Optional[str] = None, database: Optional[str] = None):
        ...
        # Log initialization
        self.session_logger.log_operation_start("GraphManager.init", {"uri": self.uri, "username": self.username})
        ...
        try:
            ...
            self.session_logger.log_operation_end(
                "GraphManager.init", duration=duration, success=True, details={"connection": "verified"}
            )
            ...
            duration = time.time() - start_time
            self.session_logger.log_operation_end(
                "GraphManager.init", duration=duration, success=False, details={"error": str(e)}
            )
    ...
    def create_code_entity(self, entity_type: str, name: str, properties: Dict[str, Any]) -> Node:
        """Create a node representing a code entity"""
        self.session_logger.log_operation_start(
            "create_code_entity", {"type": entity_type, "name": name, "properties": properties}
        )
        ...
        try:
            ...
            # Log the action
            self.session_logger.log_graph_action(action="CREATE_NODE", entity_type=entity_type, entity_name=name)

            self.session_logger.log_performance(
                metric="node_creation_time", value=duration * 1000, unit="ms", context={"entity_type": entity_type}
            )

            self.session_logger.log_operation_end(
                "create_code_entity", duration=duration, success=True, details={"node_id": node.identity}
            )
            ...
            duration = time.time() - start_time
            self.session_logger.log_operation_end(
                "create_code_entity", duration=duration, success=False, details={"error": str(e)}
            )
    ...
    def create_relationship(
        self, from_node: Node, to_node: Node, rel_type: str, properties: Optional[Dict] = None
    ) -> Relationship:
        """Create a relationship between two nodes"""
        self.session_logger.log_operation_start(
            "create_relationship",
            {"from": from_node.get("name", "unknown"), "to": to_node.get("name", "unknown"), "type": rel_type},
        )
        ...
        try:
            ...
            self.session_logger.log_performance(
                metric="relationship_creation_time", value=duration * 1000, unit="ms", context={"rel_type": rel_type}
            )

            self.session_logger.log_operation_end("create_relationship", duration=duration, success=True)
            ...
            duration = time.time() - start_time
            self.session_logger.log_operation_end(
                "create_relationship", duration=duration, success=False, details={"error": str(e)}
            )
    ...
    def find_entity(self, entity_type: str, name: str) -> Optional[Node]:
        ...
        self.session_logger.log_operation_start("find_entity", {"type": entity_type, "name": name})
        ...
        try:
            ...
            duration = time.time() - start_time
            self.session_logger.log_operation_end(
                "find_entity", duration=duration, success=False, details={"error": str(e)}
            )
    ...
    def analyze_code_structure(self, file_path: str, ast_data: Dict[str, Any]):
        ...
        self.session_logger.log_operation_start(
            "analyze_code_structure", {"file": file_path, "ast_nodes": len(ast_data.get("nodes", []))}
        )
        ...
        try:
            ...
            for node_data in ast_data.get("nodes", []):
                ...
                # Create entity node
                entity_node = self.create_code_entity(node_type, node_name, node_data.get("properties", {}))
                nodes_created += 1

                # Create relationship to file
                self.create_relationship(file_node, entity_node, "CONTAINS")
                ...
                for dep in node_data.get("dependencies", []):
                    ...
                    if dep_node:
                        self.create_relationship(entity_node, dep_node, dep["relationship"])
                        relationships_created += 1
            ...
            self.session_logger.log_operation_end(
                ...
                duration=duration,
                success=True,
                details={"nodes": nodes_created, "relationships": relationships_created},
            )
            ...
            duration = time.time() - start_time
            self.session_logger.log_operation_end(
                "analyze_code_structure", duration=duration, success=False, details={"error": str(e)}
            )
    ...
    def query_for_rag(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        self.session_logger.log_operation_start("query_for_rag", {"query": query, "limit": limit})
        ...
        try:
            ...
            # Log RAG operation (with fallback for missing method)
            relevance_scores = [r["relevance_score"] for r in processed_results]
            try:
                if hasattr(self.session_logger, "log_rag_operation"):
                    ...
                    self.session_logger.log_info(f"RAG Query: '{query}' returned {len(processed_results)} results")
            except Exception as e:
                # Don't fail the query due to logging issues
                self.session_logger.log_warning(f"RAG operation logging failed: {e}")

            self.session_logger.log_operation_end(
                "query_for_rag", duration=duration, success=True, details={"results": len(processed_results)}
            )
            ...
            duration = time.time() - start_time
            self.session_logger.log_operation_end(
                "query_for_rag", duration=duration, success=False, details={"error": str(e)}
            )
    ...
    def export_d3_format(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.session_logger.log_operation_end(
                "export_d3_format", duration=duration, success=True, details={"nodes": len(nodes), "links": len(links)}
            )
            ...
            duration = time.time() - start_time
            self.session_logger.log_operation_end(
                "export_d3_format", duration=duration, success=False, details={"error": str(e)}
            )
    ...
    def export_mermaid_format(self) -> str:
        ...
        try:
            ...
            duration = time.time() - start_time
            self.session_logger.log_operation_end(
                "export_mermaid_format", duration=duration, success=False, details={"error": str(e)}
            )
    ...
    def export_simple_format(self) -> Dict[str, Any]:
        ...
        try:
            ...
            simple_data = {
                ...
                "edges": [
                    {"from": link["source"], "to": link["target"], "label": link["type"]} for link in d3_data["links"]
                ],
                "summary": stats,
            }
            ...
            duration = time.time() - start_time
            self.session_logger.log_operation_end(
                "export_simple_format", duration=duration, success=False, details={"error": str(e)}
            )
