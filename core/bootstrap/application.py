    ...
    async def _register_services(self):
        ...
        if AIServicesInterface:
            try:
                ...
                ai_service = AIService(ai_config, self.logger)
    ...
    async def _initialize_services_in_order(self, initialization_order):
        ...
        for service_interface in initialization_order:
            try:
                ...
                self.dependency_resolver.update_service_status(
                    service_interface, 
                ...
                failed.append({
                    'service': service_interface.__name__,
                    'error': str(e)
                })
    ...
    async def _perform_health_checks(self):
        ...
        for interface, service in self.service_registry.get_all_registered().items():
            try:
                ...
                print(f"     {'✅' if health_results[interface.__name__]['status'] == 'healthy' else '⚠️ '} {interface.__name__}: {health_results[interface.__name__]['status']}")
