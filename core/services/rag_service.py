...
class RAGService(RAGServiceInterface):
    ...
    def __init__(
        ...
        # Session logging
        self.session_logger = (
            logger.create_session_logger("RAGService") if hasattr(logger, "create_session_logger") else logger
        )
    ...
    async def initialize(self, config: Dict[str, Any] = None) -> None:
        ...
        try:
            ...
            self.embedding_model = await asyncio.to_thread(SentenceTransformer, self.config.embedding_model)
            ...
            self.collection = self.chroma_client.get_or_create_collection(
                name="code_embeddings", metadata={"description": "Code embeddings for RAG"}
            )
    ...
    async def create_embeddings(self, entities: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        if not entities:
            self.logger.log_warning("No entities provided for embedding creation")
            return {"count": 0, "error": "No entities provided"}
        ...
        try:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                # Generate embeddings
                embeddings = await asyncio.to_thread(self.embedding_model.encode, batch_texts)

                # Store in ChromaDB
                self.collection.add(
                    embeddings=embeddings.tolist(), metadatas=batch_metadata, documents=batch_texts, ids=batch_ids
                )
            ...
            return {"count": 0, "error": str(e), "execution_time": execution_time}
    ...
    async def update_embeddings(self, updated_entities: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"count": 0, "error": str(e), "update_type": "failed"}
    ...
    async def get_related_context(self, entity_id: str, depth: int = 2) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.embedding_model:
                ...
                if entity_results["embeddings"]:
                    ...
                    similar_results = self.collection.query(
                        query_embeddings=[entity_embedding], n_results=min(10, len(related_entities))
                    )
            ...
            return {"entity_id": entity_id, "related_entities": related_entities, "context_depth": depth}
    ...
    async def _graph_search(self, query: str, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            graph_results = await asyncio.to_thread(self.graph_service.query_entities, criteria, limit)

            # Format results to match vector search format
            formatted_results = []
            for i, result in enumerate(graph_results):
                # Handle different result formats from graph service
                if "properties" in result:
                    # Result from query_for_rag method
                    props = result.get("properties", {})
                    formatted_results.append(
                        {
                            "content": props.get("name", ""),
                            "metadata": {
    ...
    def _combine_search_results(
        self, vector_results: List[Dict[str, Any]], graph_results: List[Dict[str, Any]], max_results: int
    ) -> List[Dict[str, Any]]:
